(async () => {
   ...
   try {
      const channelElements = [...content.querySelectorAll("ytd-browse:not([hidden]) #content-section")];
      ...
      for (d of channelElements) {
         label.innerText = `Fetching URLS... (${progress.value}/${progress.max})`;
         try {
			const channelImg = d.querySelector("img").src;

			const e = d.querySelector("#main-link.channel-link");
            ...
            channels.push([`https://www.youtube.com/feeds/videos.xml?channel_id=${channelIdMatch[1]}`, channelName, e.href, channelImg]);
         } finally {
            progress.value++;
      ...
      if (channels.length > 0) {
         ...
         let opmlText = `<?xml version="1.0" encoding="UTF-8"?>\n<opml version="1.0">\n\t<head>\n\t\t<title>YouTube Subscriptions as RSS</title>\n\t</head>\n\t<body>${channels
            .map(
               ([feed, channelName, channelUrl, channelImg]) =>
                  `\n\t\t<outline type="rss" text="${escapeXML(channelName)}" title="${escapeXML(
                     channelName
                  )}" xmlUrl="${feed}" htmlUrl="${channelUrl}" imageUrl="${channelImg}"/>`
            )
            .join("")}\n\t\t</body>\n</opml>`;
